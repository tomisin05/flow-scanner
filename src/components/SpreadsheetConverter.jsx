...
const SpreadsheetConverter = ({ onUploadComplete, tournamentId }) => {
  ...
  async function uploadPDFToFirebase(pdfBlob, filename) {
    try {
      const storageRef = ref(storage, `flows/${user.uid}/${filename}`);
      const uploadResult = await uploadBytes(storageRef, pdfBlob);
      const downloadURL = await getDownloadURL(uploadResult.ref);
    ...
    for (const worksheet of workbook.worksheets) {
      ...
      if (filteredRows.length > 0) {
        ...
        doc.autoTable({
          ...
          didDrawPage: (data) => {
            doc.setFontSize(10);
            doc.text(`Page ${data.pageNumber}`, doc.internal.pageSize.width - 20, doc.internal.pageSize.height - 10, { align: 'right' });
          }
        });
      }
      setProgress((currentPage / workbook.worksheets.length) * 100);
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      const pdfBlob = await convertSpreadsheetToPDF(file);
      const downloadURL = await uploadPDFToFirebase(pdfBlob, `${file.name.split('.')[0]}.pdf`);
  ...
  return (
    <div className="p-4 border rounded-lg shadow-sm">
      ...
      {isConverting && (
        <div className="space-y-2">
          <div className="text-blue-600">Converting spreadsheet to PDF...</div>
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              className="bg-blue-600 h-2.5 rounded-full transition-all duration-300"
              style={{ width: `${progress}%` }}
